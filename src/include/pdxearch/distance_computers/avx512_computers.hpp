...
public:
	...
	static void Vertical(const QUERY_TYPE *__restrict query, const DATA_TYPE *__restrict data, size_t start_dimension,
		...
		for (size_t dim_idx = start_dimension; dim_idx < end_dimension; dim_idx += 4) {
			...
			for (int i = 0; i < 4; ++i) { // total: 64 vectors (4 iterations of 16 vectors) * 4 dimensions each (at 1
				                          // byte per value = 2048-bits)
				// Read 64 bytes of data (64 values) with 4 dimensions of 16 vectors
				const __m512i vec2_u8 = _mm512_loadu_si512(&data[offset_to_dimension_start + i * 64]);
				const __m512i diff_u8 =
				    _mm512_or_si512(_mm512_subs_epu8(vec1_u8, vec2_u8), _mm512_subs_epu8(vec2_u8, vec1_u8));
