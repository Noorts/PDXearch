...
public:
	...
	inline static std::string NARY_ADSAMPLING_DATA =
	    std::string {CMAKE_SOURCE_DIR} + "/benchmarks/datasets/adsampling_nary/";
	...
	inline static std::string DATASETS[] = {"sift-128-euclidean",
	                                        "yi-128-ip",
	                                        "llama-128-ip",
	                                        "glove-200-angular",
	                                        "yandex-200-cosine",
	...
	inline static std::string FILTERED_SELECTIVITIES[] = {
	    "0_000135", "0_001", "0_01", "0_1",  "0_2",    "0_3",     "0_4",     "0_5",
	    "0_75",     "0_9",   "0_95", "0_99", "PART_1", "PART_30", "PART+_1",
	};
	...
	inline static int POW_10[10] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};

	inline static size_t IVF_PROBES_PHASES[] = {
	    512, 256, 128, 64, 32, 16, 8, 4, 2,
	};

	inline static float SELECTIVITY_THRESHOLDS[] = {0.005, 0.01, 0.05, 0.1,  0.15, 0.2,  0.25, 0.3,  0.35, 0.4,  0.45,
	...
	static void VerifyResult(float &recalls, const std::vector<PDX::KNNCandidate<q>> &result, size_t knn,
		...
		if (result.size() < knn) {
			std::cerr << "WARNING: Result set is not complete! Set a higher `nbuckets` parameter (Only got "
	...
	static void SaveResults(std::vector<PhasesRuntime> runtimes, const std::string &results_path,
		...
		std::sort(runtimes.begin(), runtimes.end(),
		          [](PhasesRuntime i1, PhasesRuntime i2) { return i1.end_to_end < i2.end_to_end; });
		...
		file << metadata.dataset << "," << metadata.algorithm << "," << std::setprecision(6) << avg_runtime_ms << ","
		     ...
		     << metadata.num_measure_runs << "," << all_avg_runtime_ms << "," << all_max_runtime_ms << ","
		     << all_min_runtime_ms << "\n";
		file.close();
	}
};
