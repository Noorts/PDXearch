...
public:
	...
	PDXearch(INDEX_TYPE &data_index, Pruner &pruner, int position_prune_distance, DimensionsOrder dimension_order)
	    : pdx_data(data_index), pruner(pruner), is_positional_pruning(position_prune_distance),
...
protected:
	...
	alignas(64) inline static DISTANCES_TYPE
	    pruning_distances[10240]; // TODO: Use dynamic arrays. Buckets with more than 10k vectors (rare) overflow
	...
	template <Quantization Q = q>
	void ResetDistancesVectorized(DistanceType_t<Q> *distances) {
	...
	template <Quantization Q = q>
	void EvaluatePruningPredicateVectorized(uint32_t &n_pruned, DistanceType_t<Q> pruning_threshold,
	                                        DistanceType_t<Q> *pruning_distances) {
	...
	void InitPositionsArrayFromSelectionVector(size_t n_vectors, size_t &n_vectors_not_pruned,
	                                           uint32_t *pruning_positions, DistanceType_t<Q> pruning_threshold,
	                                           DistanceType_t<Q> *pruning_distances, const uint8_t *selection_vector) {
		n_vectors_not_pruned = 0;
		for (size_t vector_idx = 0; vector_idx < n_vectors; ++vector_idx) {
			pruning_positions[n_vectors_not_pruned] = vector_idx;
	...
	template <Quantization Q = q>
	void MaskDistancesWithSelectionVector(size_t n_vectors, size_t &n_vectors_not_pruned, uint32_t *pruning_positions,
	                                      DistanceType_t<Q> pruning_threshold, DistanceType_t<Q> *pruning_distances,
	...
	static void GetClustersAccessOrderIVF(const float *__restrict query, const INDEX_TYPE &data, size_t nprobe,
		...
		for (size_t cluster_idx = 0; cluster_idx < data.num_clusters; cluster_idx++) {
			distances_to_centroids[cluster_idx] = DistanceComputer<L2, F32>::Horizontal(
			    query, data.centroids + cluster_idx * data.num_dimensions, data.num_dimensions, nullptr);
		}
	...
	void Start(const QuantizedVectorType_t<Q> *__restrict query, const DataType_t<Q> *data, const size_t n_vectors,
		...
		DistanceComputer<alpha, Q>::Vertical(query, data, n_vectors, n_vectors, 0, pdx_data.num_vertical_dimensions,
		                                     ...
		                                     quantizer.dim_clip_value, nullptr);
	...
	void FilteredStart(const QuantizedVectorType_t<Q> *__restrict query, const DataType_t<Q> *data,
		...
		if (selection_percentage > 0.20) { // TODO: 0.20 comes from the `selectivity_threshold`
			...
			DistanceComputer<alpha, Q>::VerticalPruning(
			    ...
			    pruning_positions, indices_dimensions.data(), quantizer.dim_clip_value, nullptr);
		}
		...
		while (1.0 * n_tuples_to_prune < tuples_needed_to_exit &&
			...
			if (dimension_order == SEQUENTIAL) {
				DistanceComputer<alpha, Q>::Vertical(query, data, n_vectors, n_vectors, current_dimension_idx,
				                                     ...
				                                     indices_dimensions.data(), quantizer.dim_clip_value, nullptr);
			} else {
				DistanceComputer<alpha, Q>::VerticalReordered(
				    query, data, n_vectors, n_vectors, current_dimension_idx, last_dimension_to_fetch,
				    pruning_distances, pruning_positions, indices_dimensions.data(), quantizer.dim_clip_value, nullptr);
			}
	...
	void GetClustersAccessOrderIVFPDX(const float *__restrict query) {
		...
		for (size_t centroid_idx = 0; centroid_idx < full_blocks; ++centroid_idx) {
			// TODO: Use another distances array for the centroids so I can use ResetDistancesVectorized() instead of
			// memset
			...
			DistanceComputer<L2, F32>::VerticalBlock(query, tmp_centroids_pdx, 0, pdx_data.num_dimensions, distances,
			                                         nullptr);
			MergeIntoHeap<false, F32>(tmp_cluster_indices, PDX_VECTOR_SIZE, ivf_nprobe, pruning_positions,
		...
		if (remainder_block_size) {
			memset((void *)distances, 0, PDX_VECTOR_SIZE * sizeof(float));
			DistanceComputer<L2, F32>::Vertical(query, tmp_centroids_pdx, remainder_block_size, remainder_block_size, 0,
			                                    pdx_data.num_dimensions, distances, nullptr, nullptr, nullptr, nullptr);
			MergeIntoHeap<false, F32>(tmp_cluster_indices, remainder_block_size, ivf_nprobe, pruning_positions,
			                          pruning_distances, distances, best_k_centroids);
		}
	...
	void GetL0ClustersAccessOrderPDX(const float *__restrict query) {
		...
		for (size_t centroid_idx = 0; centroid_idx < full_blocks; ++centroid_idx) {
			...
			DistanceComputer<alpha, F32>::VerticalBlock(query, tmp_centroids_pdx, 0, pdx_data.num_dimensions,
			                                            centroids_distances, nullptr);
			tmp_cluster_indices += PDX_VECTOR_SIZE;
			tmp_centroids_pdx += SKIPPING_SIZE;
		}
		std::vector<size_t> indices_sorted;
	...
	void GetL1ClustersAccessOrderPDX(const float *__restrict query, size_t n_buckets, bool safe_to_prune_space = true) {
		...
		for (size_t cluster_idx = 0; cluster_idx < clusters_to_visit; ++cluster_idx) {
			...
			if (best_k_centroids.size() < n_buckets) {
				...
				Start<F32>(query, cluster.data, cluster.num_embeddings, n_buckets, cluster.indices,
				           pruning_positions_l0, pruning_distances_l0, best_k_centroids);
				continue;
			}
			Warmup<F32>(query, cluster.data, cluster.num_embeddings, n_buckets, selectivity_threshold,
			            pruning_positions_l0, pruning_distances_l0, pruning_threshold_l0, best_k_centroids);
			Prune<F32>(query, cluster.data, cluster.num_embeddings, n_buckets, pruning_positions_l0,
			           pruning_distances_l0, pruning_threshold_l0, best_k_centroids);
			if (n_vectors_not_pruned) {
				MergeIntoHeap<true, F32>(cluster.indices, n_vectors_not_pruned, n_buckets, pruning_positions_l0,
...
public:
	...
	std::vector<KNNCandidate_t> Search(float *__restrict raw_query, uint32_t k) {
		...
		for (size_t cluster_idx = 0; cluster_idx < clusters_to_visit; ++cluster_idx) {
			...
			if (best_k.size() < k) {
				...
				Start(prepared_query, cluster.data, cluster.num_embeddings, k, cluster.indices, pruning_positions,
				      pruning_distances, best_k);
				continue;
			}
	...
	std::vector<KNNCandidate_t> FilteredSearch(float *__restrict raw_query, uint32_t k,
		...
		for (size_t cluster_idx = 0; cluster_idx < clusters_to_visit; ++cluster_idx) {
			...
			auto [selection_vector, passing_tuples] =
			    predicate_evaluator.GetSelectionVector(current_cluster, cluster_offsets[current_cluster]);
			if (passing_tuples == 0) {
				continue;
			}
			CLUSTER_TYPE &cluster = pdx_data.clusters[current_cluster];
			if (best_k.size() < k) {
				// We cannot prune until we fill the heap
				FilteredStart(prepared_query, cluster.data, cluster.num_embeddings, k, cluster.indices,
			...
			Warmup<q, true>(prepared_query, cluster.data, cluster.num_embeddings, k, selectivity_threshold,
			                pruning_positions, pruning_distances, pruning_threshold, best_k, passing_tuples,
			                selection_vector);
			Prune(prepared_query, cluster.data, cluster.num_embeddings, k, pruning_positions, pruning_distances,
			      pruning_threshold, best_k);
			if (n_vectors_not_pruned) {
	...
	std::vector<KNNCandidate_t> LinearScan(float *__restrict raw_query, uint32_t k) {
		...
		for (size_t cluster_idx = 0; cluster_idx < clusters_to_visit; ++cluster_idx) {
			...
			if (cluster.num_embeddings == PDX_VECTOR_SIZE) {
				...
				DistanceComputer<alpha, Q>::VerticalBlock(query, cluster.data, 0, pdx_data.num_dimensions, distances,
				                                          nullptr);
				...
				ResetDistancesVectorized<Q>(distances);
				DistanceComputer<alpha, Q>::Vertical(query, cluster.data, cluster.num_embeddings,
				                                     cluster.num_embeddings, 0, pdx_data.num_dimensions, distances,
