...
public:
	...
	std::pair<py::array_t<uint32_t>, py::array_t<float>> _py_Search(const py::array_t<float> &q, uint32_t k,
	                                                                uint32_t n_probe) const {
	...
	_py_FilteredSearch(const py::array_t<float> &q, uint32_t k, uint32_t n_probe,
	                   const py::array_t<uint32_t> &n_passing_tuples,
	                   const py::array_t<uint8_t> &selection_vector) const {
		auto buf = q.request(); // Get buffer info
		if (buf.ndim != 1) {
			throw std::runtime_error("Input should be a 1-D NumPy array");
		}
...
public:
	...
	// Serialize return value
	std::pair<py::array_t<uint32_t>, py::array_t<float>> _py_Search(const py::array_t<float> &q, uint32_t k) const {
		auto buf = q.request(); // Get buffer info
		if (buf.ndim != 1) {
			throw std::runtime_error("Input should be a 1-D NumPy array");
		}
	...
	std::pair<py::array_t<uint32_t>, py::array_t<float>>
	_py_FilteredSearch(const py::array_t<float> &q, uint32_t k, const py::array_t<uint32_t> &n_passing_tuples,
	                   const py::array_t<uint8_t> &selection_vector) const {
		auto buf = q.request(); // Get buffer info
		if (buf.ndim != 1) {
			throw std::runtime_error("Input should be a 1-D NumPy array");
